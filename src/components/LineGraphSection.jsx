...
const LineGraphSection = () => {
  ...
  const modeCash =  data.filter((invoice)=> invoice.paymentMode === "Cash" ).length
  console.log(modeCash);
  
  const modeUpi =  data.filter((invoice)=> invoice.paymentMode === "UPI" ).length
  const modeBank =  data.filter((invoice)=> invoice.paymentMode === "Bank" ).length
  const modeOther =  data.filter((invoice)=> invoice.paymentMode === "Other" ).length
  ...
  const formatDate = (dateStr) => {
    ...
    //format (e.g., "Jan")
    return `${day} ${month}`; // Return formatted date as "DD MMM"
  };

  const dates = data.map((item) => formatDate(item.date));

  const unpaidAmountsByDate = data.map((item) => {
    return item.payment === "Unpaid" ? item.amount : 0;
  });

  const paidAmountsByDate = data.map((item) => {
    return item.payment === "Paid" ? item.amount : 0;
  });


  const datas = {
    labels: [ "Cash","UPI", "Bank", "Other",],
    datasets: [
      {
        label: "Number of Invoices",
        data: [modeCash, modeUpi, modeBank,modeOther],
        backgroundColor: [
          "rgba(255, 99, 132, 0.5)",
          "rgba(54, 162, 235, 0.5)",
          "rgba(255, 206, 86, 0.5)",
    ...
    plugins: {
      legend: {
        ...
        labels: {
          ...
          padding: 10,
          font: {
            size: 10, // Decreased font size for legend
          },
        },
      },
    },
  ...
  return (
    <>
      <div className="flex flex-col items-center w-[30%] h-[213px] rounded-[5px]  bg-[#FFFFFF] border border-[#999999] border-opacity-[0.3]">

        <div className="flex justify-between items-center w-full h-[40px] rounded-t-[5px]  p-[10px] gap-[10px] ">
          <span className="flex w-auto h-[14px] font-bold text-[14px] leading-[18px] text-[#000000]">
            Followup Overview
          </span>

          <div className="flex w-auto h-[30px]"></div>
        ...
        <div className="flex  justify-center items-center  py-[8px]     w-[80%] h-[170px]">
          <Doughnut width={180} height={180} data={datas} options={options} />
        </div>
      </div>
    </>
  );
